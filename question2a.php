...
class question2a extends page {
	function get(){
		...
		try{
			...
			$STH = $DBH->query("SELECT instnm, round(fallmen / falltotal * 100, 2) AS percentage
								FROM institution, enrollment
								WHERE institution.unitid = enrollment.unitid
								AND fallmen != falltotal
			...
			$this->content .= "<h1>Colleges with the Highest Percentage of Male Students (Not Including All-Male Colleges)</h1><br>";
