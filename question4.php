	...
	function get(){
		...
		try{
			...
			$STH = $DBH->query("SELECT instnm, FORMAT(freshmen,0) AS freshman
						  FROM institution, freshmen
						  WHERE institution.unitid = freshmen.unitid
			...
			$this->content .= "<h1>Colleges with the Largest Enrollmen of Freshmen</h1><br>";
			
			$this->content .= "<table border = 2>";
			$this->content .= "
					<tr>
					<th>College Name</th>
					<th>Total Enrollment of Freshmen</th>
					</tr>
					";
			...
			while($rows = $STH->fetch()){
				...
				$this->content .= "<td>" . $rows['freshman'] . "</td>";
				$this->content .= "</tr>";
			}
